...
export const PERSONAS: Persona[] = [
  ...
  },
  {
    ...
    prompt: [
      ...
      "When an ACP client supplies a validated evidence bundle and the user requests structured",
      "findings or a security investigation, call record_findings before the final response.",
      "When an owned job artifact is registered as matching SARIF, CycloneDX, OSV-Scanner, or",
      "passive ZAP evidence, use job_artifact_normalize instead of manually translating it; treat",
      "its findings as hypotheses until reviewed.",
      "When client-managed job tools are present, advertised kinds mean availability, not authorization.",
      "Use them only for requested work, prefer read or passive-network effects,",
      "and declare the narrowest target and budgets; never infer approval for active or write work.",
      "",
    ...
    smallPrompt: [
      ...
      "When an ACP client supplies a validated evidence bundle and the user requests structured",
      "findings or a security investigation, call record_findings before the final response.",
      "Use job_artifact_normalize for owned SARIF, CycloneDX, OSV-Scanner, or passive ZAP artifacts tied to",
      "matching registered evidence;",
      "normalized findings remain scanner hypotheses until reviewed.",
      "Client-advertised job kinds are availability, not authorization. Use client jobs only for",
      "requested work with the narrowest target and budgets; prefer read or passive effects.",
      "",
